...
import { Trash2, ShoppingBag, Sparkles, AlertCircle } from 'lucide-react';
    ...
    const fetchRecommendations = async () => {
      if (cartItems.length > 0) {
        ...
        try {
          ...
          setRecommendations([]); // So it doesn't keep trying or show old error
        } finally {
    ...
    <div className="max-w-4xl mx-auto py-8">
      <Card className="shadow-xl">
        ...
        <CardContent>
          {cartItems.length === 0 ? (
            ...
            <div className="space-y-6">
              {cartItems.map((item) => (
                <div key={item.id} className="flex items-center gap-4 p-4 border rounded-lg shadow-sm hover:shadow-md transition-shadow">
                  <Image 
                    src={item.type === 'sticker' ? item.imageUrl! : (item as any).originalImageUrl} 
                    alt={item.name} 
                    width={80} 
                    height={80} 
                  ...
                  <div className="flex-grow">
                    ...
                    {item.type === 'custom' && <p className="text-xs text-muted-foreground">Material: {item.material}</p>}
                  </div>
        ...
        {cartItems.length > 0 && (
          <CardFooter className="flex flex-col items-stretch gap-4 pt-6 border-t">
            <div className="flex justify-between text-xl font-semibold font-headline">
              <span>Subtotal:</span>
              <span>${subtotal.toFixed(2)}</span>
            </div>
            <Button size="lg" className="w-full" disabled={!currentUser}>
        ...
        <Card className="mt-8 shadow-lg">
          <CardHeader>
             <div className="flex items-center space-x-3 mb-1">
                ...
                <CardTitle className="text-2xl font-headline">You Might Also Like</CardTitle>
             </div>
             <CardDescription>AI-powered recommendations based on your cart!</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {recommendations.map((rec, index) => (
              <Card key={index} className="p-3 text-center hover:shadow-md transition-shadow">
                <Image 
                    src={`https://placehold.co/150x150.png?text=${encodeURIComponent(rec.substring(0,15))}`}
                    alt={rec}
                    width={100}
                    height={100}
                    className="rounded-md object-cover mx-auto mb-2 border"
                    data-ai-hint="sticker design"
                />
                <p className="text-sm font-medium">{rec}</p>
                <Button variant="outline" size="sm" className="mt-2 text-xs">View</Button>
              </Card>
            ))}
          </CardContent>
        </Card>
      ...
      {recommendations && recommendations.length === 0 && !isLoadingRecs && cartItems.length > 0 &&(
         <Alert className="mt-8">
            ...
            <AlertTitle className="font-headline">No Specific Recommendations Right Now</AlertTitle>
            <AlertDescription>
